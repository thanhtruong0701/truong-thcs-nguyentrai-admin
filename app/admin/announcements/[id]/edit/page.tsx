...
import { getAnnouncementById, updateAnnouncement, uploadAnnouncementImage, uploadAnnouncementFile } from '@/app/actions/announcements'
...
export default function EditAnnouncementPage() {
      ...
      .then((ann) => {
        if (ann) {
          ...
          if (ann.imageUrl) {
            ...
            setImagePreview(ann.imageUrl)
          }
          if (ann.fileUrl) {
            setFileUrl(ann.fileUrl)
            setFileName(ann.fileName || '')
            setFileType(ann.fileType || '')
          }
        } else {
          setError('Không tìm thấy thông báo')
        }
      })
      .catch(() => setError('Lỗi khi tải thông báo'))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Lỗi khi cập nhật thông báo')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">Cập nhật nội dung thông báo</p>
      </div>
